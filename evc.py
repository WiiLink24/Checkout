    ...
    for poll in polls:
        ...
        if question_id:
            ...
            votes_query = (
                ...
                "WHERE question_id = %s AND type_cd = 0 AND wii_no IN (" + ','.join(['%s'] * len(wii_numbers)) + ")"
            )
            predictions_query = (
                "SELECT ans_cnt FROM votes "
                "WHERE question_id = %s AND type_cd = 1 AND wii_no IN (" + ','.join(['%s'] * len(wii_numbers)) + ")"
            )
            votes_rows = _run_query(votes_query, [question_id] + wii_numbers, db_url)
            predictions_rows = _run_query(predictions_query, [question_id] + wii_numbers, db_url)
            
            # Format ans_cnt for votes and predictions
            poll['votes'] = [format_ans_cnt(v.get('ans_cnt', '0')) for v in votes_rows]
            poll['predictions'] = [format_ans_cnt(p.get('ans_cnt', '0')) for p in predictions_rows]
